...
LogarithmicBin<SurfelGenerator::TransformedTriangleData> SurfelGenerator::constructSubMeshVertexDataBin(Mesh& subMesh, const std::shared_ptr<Entity> entity) {
	...
	for (int i = 0; i < subMesh.vertices.size(); i += 3) {
		Vertex& vertex0 = subMesh.vertices[i];
		Vertex& vertex1 = subMesh.vertices[i + 1];
		Vertex& vertex2 = subMesh.vertices[i + 2];

		// Transform positions
		Triangle3D triangle(modelMatrix * glm::vec4(vertex0.Position, 0.0f),
			modelMatrix * glm::vec4(vertex1.Position, 0.0f),
			modelMatrix * glm::vec4(vertex2.Position, 0.0f));
		...
		Triangle2D UVs(vertex0.TexCoords, vertex1.TexCoords, vertex2.TexCoords);
